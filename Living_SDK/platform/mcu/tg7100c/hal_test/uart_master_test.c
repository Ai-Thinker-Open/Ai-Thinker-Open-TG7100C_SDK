...
void hal_uart_api_test(int baud,int width,int parity,int stop)
{
    uint32_t count   = 0;
    uint32_t ret     = -1;
    uint32_t i    = 0;
...
int aos_hal_uart_master_test(int baud,int width,int parity,int stop,int size,int timeout)
{
    uint32_t count   = 0;
    uint32_t ret     = -1;
    uint32_t i    = 0;
...
void test_hal_uart_master(char *cmd, int type, char *data)
{
    if (strcmp((const char *)data, "'SEND_115200'\0") == 0) {
        ...
        hal_uart_api_test(115200,DATA_WIDTH_7BIT,ODD_PARITY,STOP_BITS_1);
