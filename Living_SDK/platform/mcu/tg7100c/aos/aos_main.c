...
void cmd_adc(char *buf, int len, int argc, char **argv)
{
    #if 0
    adc_dev_t dev;
    uint32_t val = 0;
    ...
    dev.port = 4;
    dev.config.sampling_cycle = 100;

    hal_adc_init(&dev);

    while (1) {
        if (0 != hal_adc_value_get(&dev, &val, 0)) {
    ...
    dev.port = 4;
    dev.config.freq = 8000;
    dev.config.data_count = 256;
    if (adc_flag == 0) {
        hal_adc_init(&dev);
        adc_flag = 1;
    }
