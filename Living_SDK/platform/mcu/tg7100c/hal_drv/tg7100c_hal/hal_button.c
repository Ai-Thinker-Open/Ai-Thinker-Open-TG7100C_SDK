...
static void clear_button_states(button_ctx_t *pstnode)
{
    ...
    if (krhino_timer_stop(&pstnode->psttimer_handler) != RHINO_SUCCESS) {
...
static void button_process(ktimer_t *pxTimer)
{
    int ret;
    int accu_time;
    button_ctx_t *pstnode;
    
    pstnode = (button_ctx_t *)container_of(pxTimer, struct button_ctx_t, psttimer_handler);
    ...
    switch (pstnode->press_stage) {
        case 0:// for debounce
        {
            ...
            // XXX(Zhuoran.rong) PLZ test this code
            if (krhino_timer_change(&pstnode->psttimer_handler, krhino_sys_tick_get(), krhino_ms_to_ticks(20)) != RHINO_SUCCESS) {
...
#else
    krhino_timer_start(&(((button_ctx_t *)pstnode->arg)->psttimer_handler));
#endif
    return;
}
...
static int hal_button_register_handler_with_dts(gpio_ctx_t *pstgpio)
{
    ...
    pstnode = aos_malloc(sizeof(button_ctx_t));
    ...
    // XXX(Zhuoran.rong) PLZ test this code
    ret = krhino_timer_create(&pstnode->psttimer_handler, "btn_proc", button_process, krhino_sys_tick_get(), krhino_ms_to_ticks(pstnode->debounce), pstnode, RHINO_TRUE);
...
#define GPIO_MODULE_MAX 5
    ...
    pinbuf = aos_malloc(GPIO_MODULE_MAX);
    if (!pinbuf) {
        log_error("mem error.\r\n");
        return;
    }
