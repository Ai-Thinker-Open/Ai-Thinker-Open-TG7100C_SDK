...
int bl_dma_irq_register(int channel, void *tc_handler, void *interr_handler, void *ctx)
{
    ...
    pstnode = aos_malloc(sizeof(struct dma_node));
    if (pstnode == NULL) {
...
void bl_dma_init(void)
{
    ...
    pstctx = aos_malloc(sizeof(struct dma_ctx));
    if (pstctx == NULL) {
...
static void _dma_test_case1(void)
{
...
#else
    first = aos_malloc(sizeof(struct bl_dma_item));
    src = aos_malloc(size);
    dst = aos_malloc(size);
#endif
    ...
    vPortFree(first);
#else
    aos_free((void *)first->src);
    aos_free((void *)first->dst);
    aos_free((void *)first);
#endif
}
