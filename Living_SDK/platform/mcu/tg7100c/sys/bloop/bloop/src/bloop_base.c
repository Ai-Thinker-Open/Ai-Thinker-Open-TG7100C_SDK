...
{
    timer->time_added = krhino_sys_tick_get();
...
copy_evt:
    ...
    if (0 == loop->bitmap_evt_sync) {
        if (utils_dlist_empty(&loop->timer_dlist)) {
            /* timer list is empty*/
            krhino_task_sem_take(RHINO_WAIT_FOREVER);
        } else {
            /* timer list is NOT empty, so wait with timeout*/
            timer = utils_dlist_first_entry(&loop->timer_dlist, struct loop_timer, dlist_item);
            time2wait = (int)timer->time_target - (int)krhino_sys_tick_get();
            if (time2wait > 0) {
              krhino_task_sem_take(time2wait);
            } else {
...
handle_timer:
    if (!utils_dlist_empty(&loop->timer_dlist)) {
        time_now = krhino_sys_tick_get(); //must use time_now to skeep the snapshot of tiemstamp
...
{
    loop->looper = krhino_cur_task_get();
}
...
void bloop_evt_set_async(struct loop_ctx *loop, unsigned int evt, uint32_t evt_map)
{
    ...
    krhino_task_sem_give(loop->looper);
}
