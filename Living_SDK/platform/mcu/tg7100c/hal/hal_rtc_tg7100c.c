...
int32_t hal_rtc_get_time(rtc_dev_t *rtc, rtc_time_t *time)
{
    ...
    time_stamp_ms += __mktime(&rtc_base);
    __gmtime_r(&time_stamp_ms, &tim);
...
int32_t hal_rtc_set_time(rtc_dev_t *rtc, const rtc_time_t *time)
{
    ...
    __rtctime_to_tm(&rtc_base, time, (rtc->config.format == HAL_RTC_FORMAT_BCD));
    return 0;
}
