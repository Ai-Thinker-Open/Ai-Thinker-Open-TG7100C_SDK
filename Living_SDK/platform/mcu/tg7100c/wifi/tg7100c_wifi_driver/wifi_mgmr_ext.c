...
static void cb_scan_complete(void *data, void *param)
{
    ...
    if (aos_sem_is_valid(&scan_sig) != 0){
        aos_sem_signal(&scan_sig);
    }
}
...
static void wifi_eth_sta_enable(struct netif *netif, uint8_t mac[6])
{
    ...
    * your ethernet netif interface. The following code illustrates it's use.*/
    ...
    netif_add(netif, &ipaddr, &netmask, &gw, NULL, &tg7100c_wifi_netif_init, &tcpip_input);
    netif->name[0] = 's';
    netif->name[1] = 't';
...
{
    strncpy(wifi_chan_conf->country_code, wifiMgmr.country_code, sizeof(wifi_chan_conf->country_code));
...
wifi_interface_t wifi_mgmr_sta_enable(void)
{
    ...
    blog_info("wifi_mgmr_sta_enable\r\n");
...
{
    blog_info("wifi_mgmr_sta_disable\r\n");
    wifi_mgmr_api_idle();
    return 0;
}
...
{
    blog_info("wifi_mgmr_sta_connect p_ssid:0x%08lx, p_psk:0x%08lx\r\n", (uint32_t)ssid, (uint32_t)psk);
...
{
    blog_info("wifi_mgmr_sta_disconnect\r\n");
...
{
    blog_info("wifi_mgmr_sta_autoconnect_enable\r\n");
...
{
    blog_info("wifi_mgmr_sta_autoconnect_disable\r\n");
...
{
    blog_info("wifi_mgmr_ap_mac_set %p.\r\n", mac);
...
int wifi_mgmr_ap_mac_get(uint8_t mac[6])
{
    ...
    bl_efuse_read_mac_factory(wifiMgmr.wlan_ap.mac);
...
{
    blog_info("wifi_mgmr_ap_sta_delete.\r\n");
...
{
    blog_info("wifi_mgmr_sniffer_enable.\r\n");
...
{
    blog_info("wifi_mgmr_sniffer_disable.\r\n");
    wifi_mgmr_api_idle();
    return 0;
}
...
{
    blog_info("wifi_mgmr_mgmt_register env = 0x%08lx, cb = 0x%08lx .\r\n", (uint32_t)env, (uint32_t)cb);
...
{
    blog_info("wifi_mgmr_mgmt_unregister env = 0x%08lx.\r\n", (uint32_t)env);
...
int wifi_mgmr_all_ap_scan(wifi_mgmr_ap_item_t **ap_ary, uint32_t *num)
{
    ...
    blog_info("wifi_mgmr_all_ap_scan.\r\n");

    if (NULL == ap_ary || NULL == num) {
        return -1;
    }

    ap_ary_p = (wifi_mgmr_ap_item_t *)aos_malloc(sizeof(wifi_mgmr_ap_item_t) * WIFI_SCAN_MAX_AP);
    if(NULL == ap_ary_p) {
        return -1;
    }
    memset(ap_ary_p, 0, sizeof(wifi_mgmr_ap_item_t) * WIFI_SCAN_MAX_AP);
    
    if(0 != aos_sem_new(&scan_sig, 0)) {
        aos_free(ap_ary_p);
        return -1;
    }
    ...
    if (0 == aos_sem_wait(&scan_sig, AOS_WAIT_FOREVER)) {
...
int wifi_mgmr_scan(void *data, scan_complete_cb_t cb)
{
    ...
    blog_info("wifi_mgmr_scan data = 0x%08lx, cb = 0x%08lx .\r\n", (uint32_t)data, (uint32_t)cb);
...
int wifi_mgmr_scan_complete_callback()
{
    ...
    blog_info("wifi_mgmr_scan_complete_callback\r\n");
...
int wifi_mgmr_scan_ap_all(wifi_mgmr_ap_item_t *env, uint32_t *param1, scan_item_cb_t cb)
{
    ...
    blog_info("wifi_mgmr_scan_ap_all\r\n");
...
{
    blog_info("wifi_mgmr_set_country_code = %s\r\n", country_code);
