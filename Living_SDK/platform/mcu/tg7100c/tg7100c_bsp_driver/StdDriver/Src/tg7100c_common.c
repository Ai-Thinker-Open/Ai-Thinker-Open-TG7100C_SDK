...
void ATTR_TCM_SECTION TG7100C_Delay_MS(uint32_t cnt)
{
    ...
    if(cnt>=1024){
        ...
        for(i=0;i<(cnt/1024);i++){
            TG7100C_Delay_US(1024*1000);
        }
    }
    if(cnt&0x3FF){
        /* delay (1-1023)ms */
        count = cnt & 0x3FF;
        TG7100C_Delay_US(count*1000);
    }
    //TG7100C_Delay_US((count<<10)-(count<<4)-(count<<3));
}
...
void* ATTR_TCM_SECTION TG7100C_MemCpy_Fast(void *pdst, const void *psrc, uint32_t n)
{
    ...
    if(((uint32_t)dst&0x3)==0&&((uint32_t)src&0x3)==0){
        TG7100C_MemCpy4((uint32_t *)dst,(const uint32_t *)src,n>>2);
        left=n%4;
        done=n-left;
        while(i<left){
