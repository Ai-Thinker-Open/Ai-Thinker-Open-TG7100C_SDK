...
lega_adc_callback_func g_lega_adc_callback_handler = NULL;
...
//    adc_base_reg[1] = REG_PL_RD(SYS_REG_BASE_XOCTRL2);
    ...
    if(adc_config->priv)
    {
        g_lega_adc_callback_handler = (lega_adc_callback_func)(adc_config->priv);
...
void AUX_ADC_IRQHandler(void)
{
    ...
    if(g_lega_adc_callback_handler)
    {
        ...
        g_lega_adc_callback_handler(vol_value);
    }
