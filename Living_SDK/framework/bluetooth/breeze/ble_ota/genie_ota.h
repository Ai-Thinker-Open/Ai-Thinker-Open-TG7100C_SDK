...
#define GENIE_CRYPTO_UNIT_SIZE 32
...
#endif
#define OTA_RECV_BUF_SIZE (CONFIG_AIS_TOTAL_FRAME * 240)
...
enum
{
    ...
    AIS_OTA_LONG_VER_RESP = 0x2A,     //Response the version info request
...
typedef struct genie_ota_ctx_s
{
    ...
    uint8_t image_type;
    // uint32_t image_ver;
    uint8_t image_ver[GENIE_CRYPTO_UNIT_SIZE - 1];
    uint32_t image_size;
...
typedef struct
{
    uint8_t image_type;
    // uint32_t ver;
    uint8_t ver[GENIE_CRYPTO_UNIT_SIZE - 1];
    uint32_t fw_size;
    uint16_t crc16;
    uint8_t ota_flag;
