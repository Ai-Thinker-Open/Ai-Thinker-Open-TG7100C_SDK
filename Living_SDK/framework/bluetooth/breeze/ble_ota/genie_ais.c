...
void genie_ais_notify(uint8_t msg_id, uint8_t cmd, uint8_t *p_msg, uint16_t len, uint8_t enc)
{
    ...
    // printf("len %d: %s", len + 4, bt_hex(&msg, len + 4));
...
static void handle_ais_disconnect(void)
{
    if (genie_ais_ctx.p_conn)
    {
        BREEZE_DEBUG("ais handle disconn:%p\r\n", genie_ais_ctx.p_conn);
...
{
    BREEZE_DEBUG("ais disconn 0x%x, state %d\r\n", reason, genie_ais_ctx.state);
...
static void genie_ais_state_update(void)
{
    ...
    printf("genie_ais_state_update state:%d\r\n", genie_ais_ctx.state);
    ...
    case AIS_OTA_VER_REQ:
    {
        if (len == 20) //This is encrypted data
        {
            // if (0 == _ais_decrypt(p_msg->payload, 16))
            {
                ...
                printf("do_decrypt len:%d data:%s\r\n", len, bt_hex(p_msg->payload, len));
