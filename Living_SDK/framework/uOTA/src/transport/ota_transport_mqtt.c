...
static int ota_gen_info_msg_module(char *buf, int len, int id, const char *ver,const char *module)
{
    int ret = 0;
    ret = ota_snprintf(buf, len, "{\"id\":%d,\"params\":{\"version\":\"%s\",\"module\":\"%s\"}}", id, ver, module);
    if (ret < 0) {
        return -1;
    }
    return 0;
...
static int ota_gen_report_msg_module(char *buf, int len, int id, int progress, const char *msg, const char *module)
{
    int ret = 0;
    ret = ota_snprintf(buf, len, "{\"id\":%d,\"params\":{\"step\": \"%d\",\"desc\":\"%s\",\"module\":\"%s\"}}", id, progress, msg?msg:NULL, module);
    if (ret < 0) {
        return -1;
    }
    return 0;
...
static int ota_trans_inform(void* pctx)
{
    ...
    if(ctx->module_report == NULL){
        ret = ota_gen_info_msg(msg, OTA_MSG_INFORM_LEN, 0, ctx->sys_ver);
        product_key = ctx->pk;
        device_name = ctx->dn;
    }else{
        ret = ota_gen_info_msg_module(msg, OTA_MSG_INFORM_LEN, 0, ctx->module_report->module_version, ctx->module_report->module_name);
        product_key = ctx->module_report->product_key;
        device_name = ctx->module_report->device_name;
    }

    if (ret != 0) {
        return -1;
    }
    ret = ota_mqtt_publish("inform", msg, product_key, device_name);
    if (0 != ret) {
...
static int ota_trans_status(int progress, void* pctx)
{
    ...
    if(ctx->upg_module_flag == 1){
        if(ctx->module_info != NULL){
            ret = ota_gen_report_msg_module(msg, OTA_MSG_REPORT_LEN, 0, progress, err, ctx->module_info->module_name);
            product_key = ctx->module_info->product_key;
            device_name = ctx->module_info->device_name;
        }else{
            return -1;
        }
    }else{
    ...
    ret = ota_mqtt_publish("progress", msg, product_key, device_name);
    if (0 != ret) {
        return OTA_TRANSPORT_FAIL;
    }
    return ret;
