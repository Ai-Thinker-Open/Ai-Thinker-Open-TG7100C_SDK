...
int user_ble_serv_request_event_handler(const int devid, const char *serviceid, const int serviceid_len,
                                        const char *request, const int request_len)
{
    ct_info("BLE Service Request Received, Devid: %d, Service ID: %.*s, Payload: %s", devid, serviceid_len, serviceid,
            request);
    return 0;
}
...
{
    ct_info("BLE property set,  Devid: %d, payload: \"%s\"", devid, request);
...
int user_ble_property_get_event_handler(const int devid, const char *request, const int request_len, char **response,
                                        int *response_len)
{
    ct_info("BLE Property Get Received, Devid: %d, Request: %s", devid, request);
