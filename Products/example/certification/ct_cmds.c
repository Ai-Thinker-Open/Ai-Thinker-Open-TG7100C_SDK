...
static void handle_linkkey_cmd(char *pwbuf, int blen, int argc, char **argv)
{
    ...
    if (argc == 1)
    {
        ...
        char product_pid[PRODUCT_KEY_LEN + 1] = {0};
        ...
        len = PRODUCT_KEY_LEN + 1;
        memset(key_buf, 0, MAX_KEY_LEN);
        memset(product_pid, 0, sizeof(product_pid));
        HAL_Snprintf(key_buf, MAX_KEY_LEN, "%s_%d", KV_KEY_PID, index);
        aos_kv_get(key_buf, product_pid, &len);

        aos_cli_printf("PK=%s.\r\n", product_key);
        aos_cli_printf("PS=%s.\r\n", product_secret);
        aos_cli_printf("DN=%s.\r\n", device_name);
        aos_cli_printf("DS=%s.\r\n", device_secret);
        aos_cli_printf("PID=%s.\r\n", product_pid);
    }
    else if (argc == 6)
    {
        int devid = 0;
        ...
        memset(key_buf, 0, MAX_KEY_LEN);
        HAL_Snprintf(key_buf, MAX_KEY_LEN, "%s_%d", KV_KEY_PID, devid);
    ...
    else if (argc == 7)
    {
        ...
        if (devid >= 0 && devid <= MAX_DEVICES_META_NUM)
        {
            ...
            memset(key_buf, 0, MAX_KEY_LEN);
            HAL_Snprintf(key_buf, MAX_KEY_LEN, "%s_%d", KV_KEY_PID, devid);
            aos_kv_set(key_buf, argv[6], strlen(argv[6]) + 1, 1);
...
#ifdef EN_COMBO_NET
static uint8_t awss_run_state = 0;
