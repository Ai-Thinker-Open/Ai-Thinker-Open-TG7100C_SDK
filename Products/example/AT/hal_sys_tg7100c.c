 ...
 *     <=0 success
 */
static int prase_tlv_kv(uint8_t *addr, uint8_t **key, uint8_t **value)
{
 ...
 *     0-success
 */
static int prase_tlv_version(uint8_t *addr, uint8_t **version)
{
 ...
 *     (>0) success
 */
static uint32_t prase_tlv_getnextoff(uint8_t *addr)
{
    uint32_t type1, length1;
    uint32_t off = 0;
...
static int ali_factory_media_info(uint8_t **addr, uint32_t *size)
{
    bl_mtd_handle_t hdr;
...
int my_ali_factory_media_print(void)
{
    bl_mtd_handle_t hdr;
    bl_mtd_info_t hdrinfo;
    int res = 0;
