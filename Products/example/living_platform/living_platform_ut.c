...
static int check_string_is_alpha(const char* str)
{
    int is_alpha = -1;
    int i = 0;
...
int living_platform_ut_tsl_unify_post(char* type, char **tsl)
{
    char *tsl_name = tsl[0];
    char *tsl_value = tsl[1];
    int type_num = -1;
    cJSON *tsl_obj = NULL;
    living_platform_ctx_t *living_platform_ctx = living_platform_get_ctx();

    if ((NULL == type)||(strlen(type) > 1)||(!isdigit(*type))) {
        printf("tsl type must be number\r\n");
        return -1;
    }

    type_num = atoi(type);
    if ((type_num < 0)||(type_num > 1)) {
        printf("tsl type %d is not supported\r\n");
        return -1;
    }

    if (!(check_string_is_alpha(tsl_name))) {
        printf("tsl name must be alphas\r\n");
        return -1;
    }

    /* Prepare json payload */
    tsl_obj = cJSON_CreateObject();
    if (tsl_obj == NULL) {
        printf("No Enough Memory\r\n");
        cJSON_Delete(tsl_obj);
        return -1;
    }
    if (0 == type_num) { /* post number */
        double number = 0.0;
        unsigned char *after_end = NULL;
        number = strtod((const char*)tsl_value, (char**)&after_end);
        if (tsl_value == after_end) {/* parse_error */
            cJSON_Delete(tsl_obj);
            printf("tsl value format error\r\n");
            return -1;
        }
        cJSON_AddNumberToObject(tsl_obj, tsl_name, number);
    } else if (1 == type_num) { /* post string */
        cJSON_AddStringToObject(tsl_obj, tsl_name, tsl_value);
    }
    char *post_payload = cJSON_PrintUnformatted(tsl_obj);
    cJSON_Delete(tsl_obj);
    printf("%s: %s\r\n", __func__, post_payload); /* show the json payload */
    int res = IOT_Linkkit_Report(living_platform_ctx->master_devid, ITM_MSG_POST_PROPERTY,
                             (unsigned char *)post_payload, strlen(post_payload));
    free(post_payload);
    return 0;
}
#endif
...
static aos_timer_t sys_timer_id;

static void sys_timer_callback(void *arg1, void *arg2)
{
    printf("%s timeout\r\n", __func__);
}
...
int living_platform_ut_systimer_test(char* action_str, char **timer_str)
{
    ...
    if (0 == strncmp("start", action_str, strlen("start"))) {
        timer_interval = atoi(timer_str[0])*1000;
        aos_timer_new_ext(&sys_timer_id, sys_timer_callback, NULL, timer_interval, 0, 1);
    } else if (0 == strncmp("stop", action_str, strlen("stop"))) {
        aos_timer_stop(&sys_timer_id);
    } else {
        printf("input error\r\n");
        return -1;
    }
